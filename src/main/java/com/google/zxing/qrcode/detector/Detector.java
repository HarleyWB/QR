...
public class Detector {
    ...
    public final DetectorResult detect(Map<DecodeHintType, ?> hints) throws NotFoundException, FormatException {
        ...
        //计算第四个点坐标
        P a = new P(info.getTopLeft().getX(), info.getTopLeft().getY());
        P b = new P(info.getTopRight().getX(), info.getTopRight().getY());
        P c = new P(info.getBottomLeft().getX(), info.getBottomLeft().getY());

        double BC = Math.pow((b.x - c.x), 2) + Math.pow((b.y - c.y), 2);
        double i1 = Math.pow(b.x, 2) + Math.pow(b.y, 2) - Math.pow(c.y, 2) - Math.pow(c.x, 2);
        double i2 = i1 / (2 * (b.x - c.x));
        double i3 = -1 * (b.y - c.y) / (b.x - c.x);
        double i4 = i2 - a.x;

        double A = i3 * i3 + 1;
        double B = 2 * (i3 * i4 - a.y);
        double C = i4 * i4 + a.y * a.y - BC;
        ...
        double X = i2 + i3 * Y;
        P d = new P(X, Y);
        ArrayList<P> mypoint = new ArrayList<>();
        mypoint.add(a);
        mypoint.add(b);
        mypoint.add(c);
        mypoint.add(d);
        for (P p1 : mypoint)
            System.out.println(Math.round(p1.x) + "   " + Math.round(p1.y));
////四个点坐标 放于points数组中
    ...
    protected final DetectorResult processFinderPatternInfo(FinderPatternInfo info)
        ...
        String s = bits.toString(" 0 ", " 1 ");

        System.out.println(s);

        try {
            FileWriter fw = new FileWriter("C:\\Users\\Harley\\Desktop\\QR.txt", false);
